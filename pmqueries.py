...
def get_hours_by_consultant(consultant_id: int):
    con = None
    try:
        con = connect()
        cursor = con.cursor(cursor_factory=RealDictCursor)

        # SQL to get hours ONLY for this specific person
        SQL = "SELECT * FROM consultanthours WHERE consultant_id = %s;"
        
        cursor.execute(SQL, (consultant_id,))
        data = cursor.fetchall()
        
        cursor.close()
        # Return a list of hours for that person
        return json.dumps({"consultant_hours": data}, default=str)
